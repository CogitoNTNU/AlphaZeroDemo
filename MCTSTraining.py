...
class MCTS:
    ...
    # Returns the average search value of the most searched move
    def get_value(self, state):
        return self.search_dict[str(state) + "-" + str(self.get_most_searched_move(state))][2]
    ...
    def get_most_searched_move(self, state, best=1):
        probs = self.get_posterior_probabilities(state)
        return self.number_to_move_func(np.where(probs == np.sort(probs)[-best])[0][0])
